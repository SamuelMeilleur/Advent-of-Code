...
for (const seed of seeds) {
  ...
  lines.forEach(mappings => {
    for (const range of mappings) {
      ...
      if (current >= parseInt(source) && current < parseInt(source) + parseInt(length)) {
...
for (const mappings of lines) {
  ...
  while (seedsRanges.length > 0) {
    ...
    for (const mapping of mappings) {
      ...
      if (start >= overlap.right || end <= overlap.left) {
        continue;
      }

      isMapped = true;
      mappedRanges.push([dest + overlap.left - source, dest + overlap.right - source])
